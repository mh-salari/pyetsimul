...
print(
    f"Fitted pupil center (refracted): ({pupil_center_img[0]:.2f}, {pupil_center_img[1]:.2f}) pixels"
)

# Get pupil boundary and fitted ellipse center without refraction
pupil_img_no_refract, pupil_center_img_no_refract = e.get_pupil_ellipse_in_camera_image(
    c, use_refraction=False
)
...
offset_refracted_vs_non_refracted = np.array(pupil_center_img) - np.array(
    pupil_center_img_no_refract
)
offset_magnitude = np.linalg.norm(offset_refracted_vs_non_refracted)
print(
    f"Offset between refracted and non-refracted centers: {offset_magnitude:.2f} pixels"
)
...
# Close the loop by adding first point at the end
pupil_closed_no_refract = np.column_stack(
    [pupil_img_no_refract, pupil_img_no_refract[:, 0:1]]
)
plt.plot(
