...
eye1.position = Position3D(-32, 150, 50)
...
eye2.position = Position3D(32, 150, 50)

# Single target point that both eyes look at (screen center)
target_point = Position3D(0, 0, 25)  # Screen at origin

# Create lights positioned around the cameras
lights = [
    Light(position=Position3D(-10, 0, 0)),  # Left side light at screen level
    Light(position=Position3D(10, 0, 0)),  # Right side light at screen level
]
...
eye_center = Position3D(
    ...
    (eye1.position.z + eye2.position.z) / 2 - 20,
)

# Calculate average position between the two eyes
eye_center2 = Position3D(
    (eye1.position.x + eye2.position.x) / 2 + 25,
    (eye1.position.y + eye2.position.y) / 2,
    (eye1.position.z + eye2.position.z) / 2 - 20,
)
...
camera2.position = Position3D(25, 0, 0)  # Second camera position
