...
print("Created Eye with default parameters:")
...
print(f"Fovea position: [{fovea_pos[0] * 1000:.3f}, {fovea_pos[1] * 1000:.3f}, {fovea_pos[2] * 1000:.3f}] mm")
...
print(f"Visual axis (normalized): [{visual_axis[0]:.4f}, {visual_axis[1]:.4f}, {visual_axis[2]:.4f}]")
...
print("For small angles (< 10°), the small angle approximation applies:")
print(f"- sin(α) ≈ α (in radians) → sin({e.fovea_alpha_deg}°) = {np.sin(alpha_rad):.6f} ≈ {alpha_rad:.6f}")
print(f"- cos(α) ≈ 1 → cos({e.fovea_alpha_deg}°) = {np.cos(alpha_rad):.6f} ≈ 1")
print(f"- cos(β) ≈ 1 → cos({e.fovea_beta_deg}°) = {np.cos(beta_rad):.6f} ≈ 1")

print("\nTherefore:")
print("Distance from optical axis ≈ R × sqrt(α² + β²)")
print("Angle kappa ≈ arctan(distance/R) ≈ sqrt(α² + β²) (for small angles)")
...
print(f"Error:                     {abs(angle_kappa_3d - angle_kappa_simple) / angle_kappa_simple * 100:.3f}%")

print("\nCONCLUSION:")
print("For practical eye tracking with typical fovea displacements,")
print("angle_kappa = sqrt(fovea_alpha_deg² + fovea_beta_deg²) is excellent!")
