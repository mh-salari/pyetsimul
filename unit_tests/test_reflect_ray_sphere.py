...
def test_basic_center_reflection():
    ...
    ray_direction = Direction3D(0.0, 0.0, 1.0)  # Ray direction (unit vector)
    ...
    expected_direction = Direction3D(0.0, 0.0, -1.0)
...
def test_angled_reflection():
    ...
    ray_direction = Direction3D(1.0, 1.0, 1.0)  # Ray direction (not unit)
    ray = Ray(ray_origin, ray_direction)
    sphere_center = Position3D(0.0, 0.0, 0.0)  # Sphere center
    ...
    expected_direction = Direction3D(-0.5773502691896256, -0.5773502691896256, -0.5773502691896256)
...
def test_homogeneous_coordinates():
    ...
    direction_homogeneous = np.array(reflected_ray.direction)
...
def test_output_properties():
    ...
    assert isinstance(reflected_ray.direction, Direction3D)
    assert intersection_result.intersects
    ...
    assert direction_array.shape == (4,)
