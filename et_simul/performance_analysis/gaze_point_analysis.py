...
def accuracy_over_gaze_points(et, eye=None, observer_pos_calib=np.array([0, 550e-3, 350e-3, 1]), 
                             observer_pos_test=None, grid_center=np.array([0, 200e-3]), 
                             dx=200e-3, dy=150e-3, grid_size=16):
    ...
    for i in range(len(X)):
        for j in range(len(Y)):
            ...
            gaze3d_measured = np.array([X[i] + U[j, i], 0, Y[j] + V[j, i]]) - observer_pos_test[:3]
    ...
    # Plot formatting
    title = f'Maximum error {errors["mtr"]["max"] * 1e3:.3g} mm  ' + \
