...
def accuracy_over_observer_positions(et, eye=None, observer_pos_calib=np.array([0, 550e-3, 350e-3, 1]), 
                                    gaze_target=np.array([0, 200e-3]), movement_range=50e-3, grid_size=16):
    ...
    for i in range(len(X)):
        for j in range(len(Y)):
            ...
            gaze3d_measured = np.array([gaze_target[0] + U[j, i], 0, gaze_target[1] + V[j, i]]) - e.trans[:3, 3]
    ...
    q = plt.quiver(X * 1000, Y * 1000, U * 1000, V * 1000, scale=150, alpha=0.8, width=0.002)
    ...
    # Plot title
    title = f'Movement ±{movement_range*1000:.0f}mm: Max {errors["mtr"]["max"] * 1e3:.3g} mm, ' + \
