...
class Eye:
   ...
   - 'across_pupil' and 'up_pupil' are two orthogonal vectors that extend
    ...
    def __post_init__(self) -> None:
        ...
        self.cornea_inner_center = self.pos_cornea - np.array([0, 0, self.r_cornea - self.r_cornea_inner - scale*1.15e-3, 0])
    ...
    def find_cr(self, l: 'Light', c: 'Camera') -> Optional[np.ndarray]:
        ...
        cr = reflections.find_reflection(l.position, c.trans[:, 3], self.trans @ self.pos_cornea, self.r_cornea)
    ...
    def get_pupil(self, N: int = 20) -> np.ndarray:
        ...
        pupil_points = (np.tile(self.pos_pupil.reshape(-1, 1), (1, N)) +
    ...
    def refract_ray_advanced(self, R0: np.ndarray, Rd: np.ndarray) -> Tuple[Optional[np.ndarray], Optional[np.ndarray], Optional[np.ndarray]]:
        ...
        I0, Id = refractions.refract_ray_sphere(O0, Od, inner_center, self.r_cornea_inner, 
                                   self.n_cornea, self.n_aqueous_humor)
    ...
    def get_pc(self, c: 'Camera', use_refraction: bool = True) -> Tuple[np.ndarray, Optional[np.ndarray]]:
