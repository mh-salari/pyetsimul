...
def draw_scene(camera, lights, eye):
    ...
    ax.plot([pos[0], pos[0] + x_axis[0]], [pos[1], pos[1] + x_axis[1]], [pos[2], pos[2] + x_axis[2]], 'r-')
    ax.plot([pos[0], pos[0] + y_axis[0]], [pos[1], pos[1] + y_axis[1]], [pos[2], pos[2] + y_axis[2]], 'g-')
    ax.plot([pos[0], pos[0] + z_axis[0]], [pos[1], pos[1] + z_axis[1]], [pos[2], pos[2] + z_axis[2]], 'b-')
    ...
    ax.plot([eye_pos[0], eye_pos[0] + x_axis[0]], [eye_pos[1], eye_pos[1] + x_axis[1]], [eye_pos[2], eye_pos[2] + x_axis[2]], 'r-')
    ax.plot([eye_pos[0], eye_pos[0] + y_axis[0]], [eye_pos[1], eye_pos[1] + y_axis[1]], [eye_pos[2], eye_pos[2] + y_axis[2]], 'g-')
    ax.plot([eye_pos[0], eye_pos[0] + z_axis[0]], [eye_pos[1], eye_pos[1] + z_axis[1]], [eye_pos[2], eye_pos[2] + z_axis[2]], 'b-')
    
    cornea_center = eye.trans @ eye.pos_cornea
    ax.plot([cornea_center[0]], [cornea_center[1]], [cornea_center[2]], 'co', markersize=8)
...
def draw_eye_anatomy(eye, target_point=None, show_axes=True, show_annotations=True):
    ...
    ax.scatter(*cornea_center_world, color='cyan', s=100, marker='o', label='Cornea Center')
    ax.scatter(*pupil_center_world, color='magenta', s=100, marker='s', label='Pupil Center')
    ...
    ax.plot([apex_world[0], optical_end[0]], 
            [apex_world[1], optical_end[1]],
            [apex_world[2], optical_end[2]], 'k--', linewidth=3, label='Optical Axis')
    
    # Show gaze direction if target provided
    if target_point is not None:
        if len(target_point) == 3:
            target_point = np.array(target_point)
        ax.plot([pupil_center_world[0], target_point[0]],
                [pupil_center_world[1], target_point[1]], 
                [pupil_center_world[2], target_point[2]], 'r-', linewidth=3, label='Gaze Direction')
        ax.scatter(*target_point[:3], color='red', s=150, marker='*', label='Gaze Target')
    ...
    if show_axes:
        ...
        for i, (color, label) in enumerate([('r', 'X'), ('g', 'Y'), ('b', 'Z')]):
            ...
            ax.plot([eye_origin[0], axis_end[0]],
                    [eye_origin[1], axis_end[1]],
                    [eye_origin[2], axis_end[2]], color=color, linewidth=2, label=f'{label}-axis')
