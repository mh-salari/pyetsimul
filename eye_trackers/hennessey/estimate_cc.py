...
def estimate_cc_hennessey(c, l, cr, r_cornea_assumed=None):
    ...
    params = {'R': [], 'alpha': np.zeros(num_lights), 'l': np.zeros(num_lights), 'r_cornea': r_cornea_assumed}
    ...
    result = minimize(lambda gx: objective_func(params, gx)[0], gx_start, 
...
def objective_func(params, gx):
    ...
    for j in range(num_lights):
        ...
        cz = gx[j] * np.tan(params['alpha'][j]) + \
            params['r_cornea'] * np.cos((params['alpha'][j] - beta) / 2)
