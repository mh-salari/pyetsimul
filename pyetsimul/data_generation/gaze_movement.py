...
class GazeMovementExperiment:
    ...
    def run_experiment(self) -> List[Dict[str, Any]]:
        ...
        for i, target in enumerate(target_positions):
            ...
            for eye in self.eyes:
                self._direct_eye_to_target(eye, target)
    ...
    def _direct_eye_to_target(self, eye: Eye, target: Position3D):
        """Direct the eye to look at a specific target position."""
        # Calculate gaze direction from eye position to target
        eye_pos = Position3D(*eye.trans[:3, 3])
        gaze_direction = target - eye_pos

        # Normalize direction vector
        direction_norm = np.linalg.norm([gaze_direction.x, gaze_direction.y, gaze_direction.z])
        if direction_norm > 0:
            gaze_direction = Position3D(
                gaze_direction.x / direction_norm, gaze_direction.y / direction_norm, gaze_direction.z / direction_norm
            )
