...
def lines_closest_point(
    ...
    """
    # Set up linear system coefficient_matrix * alpha = b for closest point parameters
    coefficient_matrix = np.array([[d1.dot(d1), -d2.dot(d1)], [d1.dot(d2), -d2.dot(d2)]])
    b = np.array([-(p1 - p2).dot(d1), -(p1 - p2).dot(d2)])

    try:
        alpha = np.linalg.solve(coefficient_matrix, b)
...
def line_intersect_2d(p11: Point2D, p12: Point2D, p21: Point2D, p22: Point2D) -> Point2D:
    ...
    coefficient_matrix = np.column_stack([np.array(dir1), -np.array(dir2)])
    b = np.array(p21 - p11)
